...
const mapDispatchToProps = (dispatch) => ({
  setUserLoginThunk: (login, email, password) =>
        ...
        <div className="grid grid-cols-1 gap-2 mt-0 mb-4 items-center justify-items-center">
          <span className="col-span-1 mb-0 mt-4 w-full">
            <span className="grid grid-cols-9 gap-3 items-center justify-items-center">
              <h2 className="col-span-3 font-bold mr-2 w-fit justify-self-end">
                Login | Email
              </h2>
              <input
                onChange={(e) => setNickname(e.target.value)}
                value={nickname}
                className="col-span-4 w-full h-8 rounded-full text-center"
              ></input>
          ...
          <div className="col-span-1">
            <button
              onClick={click}
              className="min-w-fit max-w-lg h-fit flex flex-row justify-center items-center gap-0 transition duration-50 ease-out hover:ease-in  rounded-full bg-rose-300 hover:bg-rose-200"
            >
