  ...
  const handleText = () => {
      ...
      switch (object.childNodes[x].nodeName) {
        ...
        case "#text":
          ...
          const childText = document.createTextNode(
            object.childNodes[x].textContent
          );
      ...
      for (let y = 0; y < object.childNodes[x].childNodes.length; y++) {
        switch (object.childNodes[x].childNodes[y].nodeName) {
          ...
          case "#text":
            ...
            const childText = document.createTextNode(
              object.childNodes[x].childNodes[y].textContent
            );
            textToDiv.appendChild(childText);
            object.childNodes[x].replaceChild(
              textToDiv,
              object.childNodes[x].childNodes[y]
            );
            break;
          case "BR":
          ...
          default:
            ...
            const childElem =
              object.childNodes[x].childNodes[y].cloneNode(true);
            ElemToDiv.appendChild(childElem);
            object.childNodes[x].replaceChild(
              ElemToDiv,
              object.childNodes[x].childNodes[y]
    ...
    for (let x = 0; x < object.childNodes.length; x++) {
      ...
      for (let y = 0; y < length - 1; y++) {
        const currentClass = object.childNodes[x].childNodes[y]
          ? object.childNodes[x].childNodes[y].className
          : null;
        const nextClass = object.childNodes[x].childNodes[y + 1]
          ? object.childNodes[x].childNodes[y + 1].className
          : null;
