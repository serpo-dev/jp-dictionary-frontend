...
const AuthStatus = (props) => {
  if (props.isAuth) {
    ...
    return (
      <NavLink
        to="/auth"
        className="w-24 flex flex-row items-center gap-0 transition duration-100 ease-out hover:ease-in hover:drop-shadow rounded-full bg-rose-400 hover:bg-rose-300"
      >
        <img src={authIcon} className="h-5 ml-3" />
        <p className="mt-1 mb-1 ml-1 mr-4 font-bold text-rose-900">Login</p>
      </NavLink>
    );
  }
};

const AuthStatusContainer = connect(
  mapStateToProps,
  mapDispatchToProps
)(AuthStatus);
